...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    ...
    for (int i = 0; i < substeps; ++i) {
      ...
      if (CheckCollisionCircleLine(ball.pos, ball.radius,
                                   v2_sub(left.pos, v2(0, left.radius)),
                                   v2_sub(right.pos, v2(0, right.radius)))) {
        line = v2_sub(p2, p1);
        normal_to_line = v2_normalize(v2(line.y, -line.x));
        ball.acc = v2_add(ball.acc, normal_to_line);
      }
